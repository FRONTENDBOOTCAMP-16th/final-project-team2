...
export default function TabProductsInfo({
  product,
  productContent,
  storeContent,
  reviewContent,
  ...
  return (
    <article id="product_tab" className="mx-auto mt-16 max-w-7xl px-4">
      ...
      >
        ...
        {activeTab === 'store' && (
          <div className="animate-in fade-in duration-300">{storeContent}</div>
        )}
        {activeTab === 'review' && (
          <div className="animate-in fade-in duration-300">{reviewContent}</div>
        )}
      </section>
    </article>
  )
