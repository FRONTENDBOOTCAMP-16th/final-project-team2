...
export default function InputBox({ label, placeholder, error, name, type = 'text', onChange, onFocus, onBlur, defaultValue }: InputBoxProps) {
  ...
  return (
    <div className="flex flex-col">
      <label htmlFor={uniqueId} className="text-[#474953] font-medium">
        {label}
      </label>
      <input
      ...
      {showValid && (
        <ul className="text-xs mbs-1 flex flex-col gap-1">
          <li className={isValidLength ? 'text-green-600 font-bold' : 'text-gray-500'}>{isValidLength ? '✓' : '•'} 8자 이상 입력해주세요</li>
