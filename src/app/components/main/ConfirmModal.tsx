...
export default function ConfirmModal({
  ...
  return (
    <div className=" fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 w-full h-full">
      ...
      {/* Modal Window */}
      <section
        className="animate-in zoom-in-95 relative flex min-w-90 max-w-200 flex-col overflow-hidden rounded-3xl bg-white shadow-[0_20px_50px_rgba(0,0,0,0.1)] duration-200"
        role="dialog"
        aria-modal="true"
      >
        ...
        <div className="max-h-[70vh] text-center overflow-y-auto p-4 pt-0 leading-relaxed text-[#4a4a4a]">
          {children}
        </div>

        {/* Footer Area for Buttons */}
        <div className="flex flex-row justify-end gap-3 p-6 pt-4">
          <button className='flex-1 bg-black text-white py-2 rounded-md' onClick={confirmAction}>{confirm}</button>
