...
export default function ProductTodayList({ products, image }: ProductsProps) {
  const latestProduct =
    products && products.length > 0 ? products.toSorted((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime())[0] : null;
  ...
  return (
    <>
      ...
      <div className="w-full max-w-7xl m-auto break-keep z-0 text-right">
        ...
        <p className="text-4xl font-bold text-white mt-9">{latestProduct.name}</p>

        <Link
          href={`/products/${latestProduct.category}/${latestProduct.id}`}
