...
// 조립을 위한 베이스 스키마
const roleSchema = z.enum(['USER', 'BUSINESS', 'ADMIN'],'회원 구분을 선택해주세요',)
...
const passwordSchema = z.string('유효한 비밀번호가 아닙니다').min(8, '비밀번호는 8자리 이상이여야 합니다').regex(PASSWORD_REGEX,'영어, 숫자, 특수문자(!@#$%^&*?_)가 포함 8글자가 들어가야 합니다',)
...
// 실제 사용되는 비밀번호 변경 스키마
export const resetPasswordSchema = baseResetPassangeSchema.superRefine((data, context) => {
  // 값이 일치하지 않을때 메세지 출력
  if (data.password !== data.confirmPassword) {
    context.addIssue({
      code: "custom",
