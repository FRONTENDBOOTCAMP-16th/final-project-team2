...
export const getNotices = async (pages: number): Promise<NoticeResponse> => {
  ...
  const { count: totalCount, error: countError } = await supabase.from('notices').select('*', { count: 'exact', head: true }).eq('important', false);
  ...
  const [importantResult, normalResult] = await Promise.all([
    supabase.from('notices').select('*, writer:writer_id (nickname)').eq('important', true).order('created_at', { ascending: false }),
    supabase
      .from('notices')
